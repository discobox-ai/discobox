...
from the in-sandbox `sandbox-agent` API.
...
| `api/model` | Stable aliases for pool-local sandbox operation schema types (generated by `api/internal/genmodelaliases`), plus hand-written wire constants both sides import, such as the `ErrorTypeSandboxArchived` problem `type`. See [`api/model/DESIGN.md`](api/model/DESIGN.md). |
| `cmd/discobox-pool-agent` | Pool agent binary entrypoint. The same binary runs the `proxy` and `buildkit-mediator` systemd units as subcommands. |
...
3. The agent loads its Ed25519 identity key from durable pool storage
   (`layout.PoolIdentityKey`), generating and storing one on first boot.
4. A newly generated key is registered at `/api/pools/register` with project
   ID, pool ID, the single-use bootstrap token, and the public key; the control
   plane redeems the token against that pool and stores the key. A key loaded
   from disk skips registration and spends no token
   ([ADR 0063](../docs/adr/0063-a-pool-agent-keeps-its-identity-key-and-registers-once.md)).
5. Every later control-plane call carries a pool assertion the agent signs
   itself (`poolauth`: PASETO v4.public, audience `discobox-control-plane`,
   5-minute TTL). The control plane issues no runtime token.
...
measured per report. Richer pressure/condition details can be sent as an opaque
...
sandbox's counters, and the disk under all of them
([ADR 0071](../docs/adr/0071-resource-accounting-is-a-pool-agent-differenced-report.md)).
...
disjoint measurements added. Services are never derived by subtracting the
...
- **Deltas**, from the Docker event stream — `start`, `die`, `stop`, `destroy`.
  ...
  nothing else, and would go on being reported as running.
- **A complete sync**, on agent start and every 60s, listing every sandbox
...
Providers reach this server through a driver-provided HTTP client lease
(`AcquirePoolAgentClient`) that carries both the transport and the base URL, so
callers never dial the agent themselves. The agent serves plain HTTP on the
listener `wire.Listen` opens from `DISCOBOX_AGENT_LISTEN_URL`: a TCP port by
default, AF_VSOCK for the libkrun and vz microVMs, which therefore open no TCP
listener. Outbound calls — registration, status, state and resource reports,
proxy secret resolution — use `wire.HTTPClient` on the control-plane URL, which
dials TCP, host CID 2 over VSOCK, or the Unix socket `discobox-cp-relay` serves
in a wslc guest. HTTP routes and authentication are identical across
transports.
...
- Source-scoped durable data is a separate pool-owned lifecycle boundary:
  ...
  tree.
- Every sandbox has primary source data, always at
  `/.discobox/data-per-source/primary` (`sandboxconfig.PrimarySourceSlug`)
  whatever the primary's own slug, because that fixed path is what harness
  images read; the control plane never gives that name to a reference. When
  the primary source has no key — the sandbox has no primary source, or no
  origin to derive one from — the pool binds a private directory in its place:
  no source is a private source. It lives inside the sandbox's own `data` tree
  (`layout.SandboxSourceData`, `data/.discobox/data-per-source/primary`), so
...
- Forward the sandbox user; never complete it. The pool host cannot resolve a
  ...
  §§4–5). This is structural rather than a rule to remember: `resolveSandboxUser`
...
- Absent is absent, all the way onto the wire. `DISCOBOX_USER_*` is stamped only
  ...
  identity it resolved instead. Plain ints could not say so: absent would
  arrive as `0` and chown the primary source tree to root, in the case where
  the sandbox is least likely to be running as root. `-1` survives
...
a bare shell. There is no default image: any stand-in that cannot host a
sandbox agent would turn a malformed request into a container that never
answers, instead of a failure naming what was wrong. Validation runs before any
container work, so a malformed request costs nothing.
...
Replacing a container makes the old one vanish, and its `destroy` reaches the
control plane as an ordinary `stopped` delta on the state channel, followed by
the new container's `start` and the end-of-create read-back, all ordered by boot
ID and sequence. This module therefore needs no suppression and no in-flight
bookkeeping of its own — it reports what it saw.
...
not an arrival, and is kept as an unknown age; see ADR 0143.
