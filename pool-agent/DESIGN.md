...
| `cmd/discobox-vsock-guest` | Local-VM guest services, built into the VM image: VSOCK-to-Docker byte splice and orderly shutdown endpoint. The shutdown endpoint also serves macOS guests, where it runs `shutdown(8)`: Virtualization.framework's own stop request reaches a macOS guest as a power-button press, which it answers by sleeping. |
...
| `vsock` | Guest AF_VSOCK listener and host-CID HTTP transport primitives. Linux uses `mdlayher/vsock`, which implements nothing else; darwin (a macOS guest) has its own listener over `x/sys/unix`, and does not dial yet. |
