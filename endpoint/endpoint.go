...
// [Parse] leaves it unresolved and [Resolve] looks it up (ADR 0113 §1).
//
...
func Parse(raw string) (Endpoint, error) {
	...
	case SchemeDiscobox, "iroh":
		...
		//
		// discobox:// also names a server by host (ADR 0113 §1). A peer ID is
...
// (ADR 0113 §1). It is an http endpoint like any other, so it is settled here
