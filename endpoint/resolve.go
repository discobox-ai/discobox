...
// the way every other Discobox surface writes one (ADR 0116 §1).
...
// an https server (ADR 0116 §1). Every other endpoint comes back exactly as
...
// rather than by peer ID (ADR 0116 §1). An IP address, or any host with a
