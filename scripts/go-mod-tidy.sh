...
#
# Called by `go tool task tidy` and by the Discobot go.mod file hook. With no
# arguments every workspace module is tidied; arguments are changed file paths,
...
# sync hiccup should not block the caller.
