...
const (
	...
	ProjectEventNameResourceChanged = EventTypeResourceChanged
	ProjectEventNameResourceListed  = EventTypeResourceListed
	ProjectEventNameListStart       = "list-start"
	ProjectEventNameListFinish      = "list-end"
)

// ProjectEvent is the public project stream event shape.
...
func (c *Client) SubscribeProjectEvents(ctx context.Context, projectID string, params ProjectEventsParams) (*ProjectEventStream, error) {
	base := *c.requestURL(ctx)
	u := base.JoinPath("projects", projectID, "stream")
	u.Scheme = websocketScheme(u.Scheme)

	conn, resp, err := websocket.Dial(ctx, u.String(), &websocket.DialOptions{HTTPClient: c.httpClient()})
...
func (c *Client) httpClient() *http.Client {
	if client, ok := c.cfg.Client.(*http.Client); ok {
		return client
	}
	return &http.Client{Transport: clientRoundTripper{client: c.cfg.Client}}
}
...
type ResourceChangedEvent ProjectEvent

func (*ResourceChangedEvent) projectEventData() {}

type ResourceListedEvent ProjectEvent
