...
type Executor interface {
	...
	// cause a retry depending on the job's MaxAttempts. The result is persisted
	// with the job whether the attempt completes, fails, or is canceled.
...
// WithConcurrency sets the maximum number of jobs of the registered type that
// can run at the same time on one dispatcher. Values less than one should be
// treated as one by Register.
