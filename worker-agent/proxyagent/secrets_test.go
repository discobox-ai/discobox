...
func TestResolverApproved(t *testing.T) {
	...
	if err := WriteResolveContext(hostDirFor, srv.URL, "worker-1", "tok-123"); err != nil {
	...
	if gotPath != "/api/workers/worker-1/resolve-sandbox-secret" {
...
func TestResolverPendingIsDenied(t *testing.T) {
	...
	defer srv.Close()
	_ = WriteResolveContext(hostDirFor, srv.URL, "worker-1", "tok")
