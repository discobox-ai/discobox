...
func (d *Driver) containerBootConfig(boot vm.BootConfig) vm.BootConfig {
	if strings.TrimSpace(boot.Env[workerbootstrap.EnvControlPlaneURL]) != "" {
		return boot
	}
	...
	env[workerbootstrap.EnvControlPlaneURL] = defaultDockerControlPlaneURL()
	boot.Env = env
	return boot
}
...
func (d *Driver) AcquireWorkerHTTPClient(ctx context.Context, workerID string) (*sandbox.HTTPClientLease, error) {
	...
	authToken := envValue(inspect.Container.Config.Env, workerbootstrap.EnvBootstrapToken)
