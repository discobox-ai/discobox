...
- Authorizers should assert what a route is, not what it is not. Avoid negative checks like "not a worker route" as authorization criteria because new or misspelled routes can be unintentionally allowed.
...
- Do not authorize from request-body fields. Use principal, method, path parameters, query parameters, headers, and resource ownership loaded from those attributes. The only exception is `POST /api/workers/register`, which may redeem body-provided bootstrap identity plus a one-time bootstrap token before a worker principal exists.
- Worker authentication must derive `WorkerID` from the validated credential, never from the URL or request body.
- Worker status authorization must compare the authenticated worker principal to the path `workerId` before updating state.
...
- Preserve public-path bypass behavior only for docs/openapi routes.
