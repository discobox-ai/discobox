...
- Do not store worker private keys centrally.
...
- Do not base authorization decisions on request-body fields; authorize from the authenticated principal and request attributes such as method, path parameters, query parameters, headers, and resource ownership loaded from those attributes. The only exception is worker bootstrap registration, which may redeem body-provided project/sandbox identity plus a one-time bootstrap token before a worker principal exists.
