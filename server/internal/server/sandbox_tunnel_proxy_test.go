...
// Both tunnels ride the same lease chain to the pool agent, each asking for its
// own scope and nothing broader.
func TestSandboxTunnelRoutesProxyToSandboxAgent(t *testing.T) {
	for _, test := range []struct {
		suffix string
		scope  string
	}{
		{suffix: "/tcp/attach", scope: poolagentauth.ScopeTCPConnect},
		{suffix: "/udp/attach", scope: poolagentauth.ScopeUDPConnect},
	} {
		t.Run(test.suffix, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				wantPath := "/api/project/" + projectID + "/pool/pool-1/sandboxes/sandbox-1" + test.suffix
			...
			req := scopedUserRequest(ctx, http.MethodGet, "/api/projects/"+projectID+"/sandboxes/sandbox-1"+test.suffix+"?host=127.0.0.1&port=8080", nil, test.scope)
			router.ServeHTTP(resp, req)

			if resp.Code != http.StatusOK {
				t.Fatalf("GET %s status = %d, body = %s", test.suffix, resp.Code, resp.Body.String())
			}
			...
			if !slices.Equal(stubs.sandboxScopes, []string{test.scope}) {
				t.Fatalf("sandbox scopes = %#v, want only %q", stubs.sandboxScopes, test.scope)
			}
		})
	}
}
