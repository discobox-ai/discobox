...
func TestSandboxReconcileExecutorAssertsGenerationBeforeExecute(t *testing.T) {
	...
	executor := sandboxes.NewSandboxReconcileExecutor(reconciler)
	job, err := orchestration.JobFromPayload(sandboxes.SandboxReconcilePayload{
		ProjectID:  "project-1",
		SandboxID:  "sandbox-1",
		Generation: 1,
...
func TestWorkerReconcileExecutorAssertsGenerationBeforeExecute(t *testing.T) {
	...
	executor := workers.NewWorkerReconcileExecutor(reconciler)
...
func TestProviderReconcileExecutorExecutesProviderReconciler(t *testing.T) {
	...
	executor := providers.NewProviderReconcileExecutor(reconciler)
	job, err := orchestration.JobFromPayload(providers.ProviderReconcilePayload{
		ProjectID:  "project-1",
		ProviderID: "provider-1",
	}, orchestration.QueueConfig{DefaultMaxAttempts: 1})
	if err != nil {
...
func TestWorkerReconcileExecutorNotifiesTerminalHandler(t *testing.T) {
	...
	executor := workers.NewWorkerReconcileExecutor(&fakeWorkerReconciler{}, handler)
	job, err := orchestration.JobFromPayload(workers.WorkerReconcilePayload{
		ProjectID:  "project-1",
		ProviderID: "provider-1",
		WorkerID:   "worker-1",
