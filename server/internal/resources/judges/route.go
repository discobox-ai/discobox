...
// Routing a job to the judge (ADR 0149 §2).
//
...
// credential its discobox asked for is not coming (ADR 0149 §1).
func (s *Service) Judge(ctx context.Context, poolID string, ask services.JudgeAsk) (judge.Answer, error) {
	...
	if !s.enabled {
		...
		// means no credential goes out (ADR 0149 §4).
	...
	// discobox that asked (ADR 0149 §4: the reason is what a discobox learns).
	...
	// One ask is bounded here as well as at the judge (ADR 0149 §2): a caller
	...
	// grant can be revoked inside it (ADR 0149 §4). The check is the same one
