...
func ensureDefaultPool(ctx context.Context, appStore *store.Store, defaultProvider *model.SandboxProviderInstance) error {
	...
	// The same pool runs the project's judge (ADR 0148 §1). It is recorded
	...
	// operating system asks this one's judge rather than running its own.
	project.JudgePoolID = pool.ID
