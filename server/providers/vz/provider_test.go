...
func TestValidateAcceptsLocalGuestArtifacts(t *testing.T) {
	...
	resolver, err := guestResolver(Config{GuestImageDir: hostGuestDir()}, dockerworker.ServerDefaults{})
	if err != nil {
...
func TestGuestImageBuildSpecPointsAtTheGuestThisDriverBoots(t *testing.T) {
	...
	guest, err := guestResolver(Config{GuestImageLocalDir: local}, dockerworker.ServerDefaults{})
	if err != nil {
		t.Fatalf("guestResolver: %v", err)
	}
...
func TestGuestImageBuildSpecDeclinesWithoutALocalDirectory(t *testing.T) {
	...
	guest, err := guestResolver(Config{GuestImageDir: override}, dockerworker.ServerDefaults{})
	if err != nil {
		t.Fatalf("guestResolver: %v", err)
	}
	driver := &Driver{guest: guest}
