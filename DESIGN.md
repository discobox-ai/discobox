...
| [`tarsums`](tarsums) | A tar archive whose last member is a `SHA256SUMS` in `sha256sum`'s own format, and a reader that ends it with `io.EOF` only once that member has arrived and matched. Shared because both hops of a discobox transfer write and verify one — the pool agent for a sandbox's tree, the server for a `.dbox` — and each writes its own sums only after verifying what it read. See [ADR 0123](docs/adr/0123-a-discobox-is-exported-as-its-spec-and-its-durable-tree.md) §8. |
