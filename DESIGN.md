...
| [`judge`](judge) | The question Discobox puts to a model before a credential is used, and the answer it will accept back: the job — a command, or a request the proxy observed — the system prompt and schema, and the strict decoding that refuses anything readable as permission nobody gave. A request's body is described rather than sent, and the judge asks to be shown it, which is why an answer may allow, refuse, or ask, and an allow may stand for a route (ADR 26-09-25-428). Shared because the caller asking, the runtime prompting, and the control plane recording a verdict must agree on every word of it. See [`judge/DESIGN.md`](judge/DESIGN.md) and [ADR 26-09-22-838](docs/adr/26-09-22-838-a-dedicated-pool-harness-judges-commands-and-credential-bearing-requests.md). |
| [`wellknown`](wellknown) | The registry of well-known credentials: a reverse-DNS ID (`com.github.api`), the hosts it is sent to — each covering the hosts beneath it, so `github.com` answers for `api.github.com` too — the variable it is delivered in, and the command that prints it on a logged-in machine, offered to a person who stores it (ADR 26-09-25-122), so an agent asks for one by ID. Shared because the pool's credential broker expands an ID into a request and the control plane checks it and binds the project secret marked with it. |
| [`endpoint`](endpoint) | How a client reaches the control plane and how the control plane listens, resolved from a URL scheme — and, for a `discobox://` address that names its server by DNS name, from the `_discobox` TXT record `Resolve` looks up before a dial (ADR 0116). It also owns the grammar of a discobox's own address, `discobox://<server>/<discobox>`. Shared because the CLI and the server must agree on what an endpoint means, and because `git`, websockets, and the generated client all reach the server through the one client it builds. The pool-agent hop is resolved separately by [`pool-agent/wire`](pool-agent/wire). It also owns `Diagnose`, which reports reaching a server layer by layer rather than as the one error the top of the stack produces, and the transport logging behind `discobox --iroh-log` and the server's `iroh.logLevel` — both live here because the layers they describe are this package's, and nothing above it can see them. |
...
| [`releasemanifest`](releasemanifest/DESIGN.md) | Full release build inventory: source revision, CLI and server downloads with platform/size/digest metadata, and named runtime/harness image roles. Also consumed by development builds through `DISCOBOX_RELEASE_MANIFEST`. |
...
| [`tarsums`](tarsums) | A tar archive whose last member is a `SHA256SUMS` in `sha256sum`'s own format, and a reader that ends it with `io.EOF` only once that member has arrived and matched. Shared because every hop of a discobox transfer writes and verifies one — the sandbox agent's export mode and the pool agent for a sandbox's tree, the server for a `.dbox` — and each writes its own sums only after verifying what it read. See [ADR 0123](docs/adr/0123-a-discobox-is-exported-as-its-spec-and-its-durable-tree.md) §8. |
| [`sandboxtree`](sandboxtree) | A sandbox's durable tree as a tar: its subtrees (`data`, `sources`, `origins`), the walk that writes a directory into one — hard links stored once, sockets and fifos skipped, a file that changed length ending the archive — and the check that an entry's name is inside the subtrees a writer may send. Shared because two processes write one tree: the sandbox agent's export mode writes `data` and `sources`, and the pool agent verifies that stream, re-emits it and adds its own `origins` ([ADR 0129](docs/adr/0129-the-sandbox-agent-reads-the-tree-an-export-carries.md)). |
...
| [`sandboxmeta`](sandboxmeta) | A sandbox's meta — its description and tags: the file inside the sandbox that holds them (`~/.discobox/meta.json`, their system of record), the rules each must satisfy, how a change applies, and the tag selector grammar listings filter by. Shared because the sandbox agent reads and writes the file, the server validates what it caches and filters on, and the CLI parses selectors ([ADR 0136](docs/adr/0136-a-sandboxs-meta-lives-in-the-sandbox-and-the-server-caches-it.md)). |
