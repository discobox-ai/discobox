...
| [`frontmatter`](frontmatter) | The `.discobox/` file format: a script with a YAML metadata block delimited by `---`, `#---` or `//---`, and a stable id derived from its filename. Shared because it is a convention of that directory rather than a detail of any one reader — service definitions (`sandbox-agent/services`) and hook definitions in `discobox-ai/hooks` are the same format. It normalizes key spelling and value shape and nothing else; which keys are required and what they mean belongs to the domain reading them. |
