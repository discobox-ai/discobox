...
func BuildBootConfig(input BootInput) BootConfig {
	env := map[string]string{
		workeragent.EnvControlPlaneURL: input.ControlPlaneURL,
		workeragent.EnvTenantID:        firstNonEmpty(input.WorkerBootstrap.TenantID, input.Ref.TenantID),
		workeragent.EnvProjectID:       firstNonEmpty(input.WorkerBootstrap.ProjectID, input.Ref.ProjectID),
