...
func TestBuildBootConfigIncludesTenantAndWorkerRegistration(t *testing.T) {
	...
	if boot.Env[workeragent.EnvTenantID] != "tenant-1" {
		t.Fatalf("tenant env = %q", boot.Env[workeragent.EnvTenantID])
	}
...
func TestProviderCreatesVMWithBootConfigAndState(t *testing.T) {
	...
	if driver.createdSpec.Boot.Env[workeragent.EnvTenantID] != "tenant-1" {
		t.Fatalf("created tenant env = %q", driver.createdSpec.Boot.Env[workeragent.EnvTenantID])
	}
