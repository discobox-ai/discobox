...
type Store interface {
	// EnsureActiveJobForPayload returns an active job for the payload's resource,
	// creating one when needed. Payloads that implement DuplicateAllower and
	// return true should always create a separate job row. The bool return is
	// true only when a new row was created.
	EnsureActiveJobForPayload(ctx context.Context, payload Payload, cfg QueueConfig) (*Job, bool, error)
