...
| [0114](0114-a-discobox-address-names-a-server-and-a-discobox.md) | A Discobox address names a server and a discobox, and a client lists every server it knows | Accepted (supersedes [0097](0097-a-discobox-address-is-versioned-and-names-no-transport.md) §1's exact-concatenation address) |
| [0115](0115-every-server-has-a-peer-id.md) | Every server has a peer ID, whatever it listens on | Accepted (supersedes [0098](0098-a-server-serves-its-own-peer-id.md) §2 and [0052](0052-iroh-is-an-optional-endpoint-scheme.md) §6's key loaded only for a bound endpoint) |
