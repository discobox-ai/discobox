...
| ADR | Title | Status |
| --- | --- | --- |
| [0001](0001-sandbox-origin-and-remote-source-push.md) | Sandbox origin and remote source push | Accepted (§1's `Origin.ProjectPath` and derivation of `OriginKey` superseded by [0111](0111-the-origin-is-the-client-and-its-key-names-where-the-source-came-from.md)) |
...
| [0040](0040-discobox-images-are-reclaimed-by-label-and-local-age.md) | Discobox images are reclaimed by label and local tag age | Accepted (§2's `LastTagTime`-on-build premise superseded by [0143](0143-an-arrival-no-later-than-the-build-is-an-unknown-age.md)) |
...
| [0078](0078-the-wsl-proxycommand-is-quoted-for-sh.md) | The WSL bridge, corrected: quoting, the key's ACL, and both sides written | Accepted (amends [0074](0074-a-wsl-cli-writes-the-ssh-config-windows-reads.md) §2, §3; §2's ACL superseded by [0102](0102-a-mirrored-key-carries-the-acl-ssh-reads.md)) |
...
| [0082](0082-a-stopped-sandbox-tracks-its-harness-image.md) | A stopped sandbox tracks its harness image | Accepted (supersedes [0021](0021-upgrade-is-a-re-pin-and-preserves-power-state.md) §2; §2's `ready`-only eligibility superseded by [0121](0121-a-stopped-sandbox-that-failed-tracks-its-harness-image-too.md)) |
...
| [0089](0089-the-bare-command-is-a-run-and-costs-unknown-command.md) | The bare command is a run, and costs "unknown command" | Accepted (§4's positional prompt and §5's `ArbitraryArgs` superseded by [0100](0100-the-prompt-is-a-flag-and-the-root-takes-no-words.md); the `discobox run` spelling superseded by [0133](0133-the-command-that-makes-a-discobox-is-new.md); §3's root half and §4 superseded by [26-09-25-027](26-09-25-027-only-new-makes-a-discobox.md); §§1–2 stand) |
...
| [0105](0105-a-release-is-cut-as-a-prerelease-and-blessed-stable-later.md) | A release is cut as a prerelease and blessed stable by hand | Accepted (§2 amended by [0110](0110-the-install-script-is-a-release-asset-and-a-channel-names-which-one-runs.md) — an explicit prerelease reaches the opt-in `edge` channel) |
| [0106](0106-a-consumer-points-at-the-asset-mirror-only-if-it-can-fall-back.md) | A consumer points at the asset mirror only if it can fall back | Accepted (amends [0099](0099-the-cli-downloads-the-server-it-starts.md) §3 — an asset names an ordered list of URLs; §4 superseded by [0110](0110-the-install-script-is-a-release-asset-and-a-channel-names-which-one-runs.md) for the installer's two addresses) |
...
| [0108](0108-a-sandbox-stops-itself-when-its-terminals-go-quiet.md) | A sandbox stops itself when its terminals go quiet | Accepted (§2's title change superseded by [0124](0124-a-terminal-is-active-while-what-it-shows-changes.md)) |
...
| [0111](0111-the-origin-is-the-client-and-its-key-names-where-the-source-came-from.md) | The origin is the client, and its key names where the source came from | Accepted (supersedes [0001](0001-sandbox-origin-and-remote-source-push.md) §1's `Origin.ProjectPath` and derivation of `OriginKey`) |
...
| [0116](0116-a-discobox-address-names-a-server-and-a-discobox.md) | A Discobox address names a server and a discobox, and a client lists every server it knows | Accepted (supersedes [0097](0097-a-discobox-address-is-versioned-and-names-no-transport.md) §1's exact-concatenation address; §3's `discobox servers` spelling superseded by [0119](0119-registered-servers-are-admin-remote.md); §4's primary rule superseded for the launcher by [0122](0122-a-window-that-polls-lists-the-servers-that-answer.md); §4's primary-only inbox and config screens superseded by [0131](0131-the-launcher-answers-every-servers-credential-requests-and-names-the-server-its-config-screens-edit.md); §3's primary sources superseded by [0135](0135-the-primary-server-is-recorded-by-admin-remote-primary.md)) |
| [0117](0117-every-server-has-a-peer-id.md) | Every server has a peer ID, whatever it listens on | Accepted (supersedes [0098](0098-a-server-serves-its-own-peer-id.md) §2 and [0052](0052-iroh-is-an-optional-endpoint-scheme.md) §6's key loaded only for a bound endpoint) |
| [0118](0118-whoever-serves-an-api-is-strict-and-whoever-reads-it-is-tolerant.md) | Whoever serves an API is strict, and whoever reads its responses is tolerant | Proposed (deferred until its revisit condition holds) |
| [0119](0119-registered-servers-are-admin-remote.md) | Registered servers are `discobox admin remote` | Accepted (supersedes [0116](0116-a-discobox-address-names-a-server-and-a-discobox.md) §3's command spelling) |
...
| [0125](0125-tools-are-declared-in-files-the-way-services-are.md) | Tools are declared in files, the way services are | Accepted (§4's `/` fallback for `{workdir}` superseded by [0142](0142-a-host-tool-with-no-working-tree-opens-the-working-root.md)) |
| [0126](0126-a-sandbox-does-not-share-a-host-or-a-filesystem-with-its-pool.md) | A sandbox does not share a host or a filesystem with its pool | Accepted (supersedes [0026](0026-local-source-origin-is-bind-mounted-live-into-the-sandbox.md) §§1-2's bind into the sandbox) |
...
| [0140](0140-a-discobox-reaches-the-discobox-api-through-its-pool-with-a-fixed-role.md) | A discobox reaches the discobox API through its pool, with a fixed role | Accepted (the rejection of authority by creator superseded, for source delivery, by [26-09-24-630](26-09-24-630-a-discobox-delivers-the-source-of-the-discoboxes-it-creates.md)) |
...
| [0142](0142-a-host-tool-with-no-working-tree-opens-the-working-root.md) | A host tool with no working tree opens the working root | Accepted (supersedes [0125](0125-tools-are-declared-in-files-the-way-services-are.md) §4's `/` fallback) |
...
| [26-09-25-428](26-09-25-428-the-judge-may-let-its-allow-stand-for-a-route.md) | The judge may let its allow stand for a route | Accepted (supersedes [26-09-22-838](26-09-22-838-a-dedicated-pool-harness-judges-commands-and-credential-bearing-requests.md) §7's rules-only standing allow) |
| [26-09-25-146](26-09-25-146-sandbox-terminals-share-the-desktops-socket-activated-session-bus.md) | Sandbox terminals share the desktop's socket-activated session bus | Accepted |
| [26-09-25-027](26-09-25-027-only-new-makes-a-discobox.md) | Only `new` makes a discobox; the bare command takes none of its flags | Accepted (supersedes [0089](0089-the-bare-command-is-a-run-and-costs-unknown-command.md) §3's root half and §4, [0100](0100-the-prompt-is-a-flag-and-the-root-takes-no-words.md) §2 and §4's lead spelling, and [0133](0133-the-command-that-makes-a-discobox-is-new.md) §3's bare-command `addRunFlags` and its `discobox -p` consequence) |
| [26-09-25-122](26-09-25-122-a-token-may-expire-and-suggest-the-command-that-renews-it.md) | A token may expire, and suggest the command that renews it | Accepted |
| [26-09-26-909](26-09-26-909-pi-and-omp-are-harnesses-and-omp-is-signed-in-through-its-own-importer.md) | Pi and Oh My Pi are harnesses, and omp is signed in through its own importer | Accepted |
