...
- **Status**: Accepted
- **§1's exact-concatenation address superseded by**: [0114](0114-a-discobox-address-names-a-server-and-a-discobox.md) — an address may also name a server by host, and a discobox on it; resolution moves from `Parse` to a `Resolve` step before the dial. The peer-ID format, and §§3-7, stand.
- **Date**: 2026-09-08
