...
- Every harness image provides **`/usr/local/bin/discobox-prompt`**, a one-shot
  ...
  answering on stdout with only the final response (one JSON document when a
  schema is supplied) and exiting 0 only when the model answered. `--model`
  ...
  its callers resolve it by name. The dedicated pool judge invokes it through sandbox-agent for both credential
  command checks and proxy request checks, always with `--no-tools`. Codex uses
  native schema validation and final-message capture so its CLI transcript never
  enters the verdict parser; approval policy precedes the `exec` subcommand. A
  harness with no wrapper (`shell`) cannot answer judge requests.
