...
// `pi --print` says only what the model said, but a model asked for JSON may
// still fence it, so a schema'd answer goes through the image's answer helper
// (harness/DESIGN.md).

func TestPiPromptNormalizesTheAnswer(t *testing.T) {
...
// A pi that fails is a wrapper that failed, whatever it printed first.
func TestPiPromptFailsWhenPiDoes(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("the image's scripts run on Linux")
	}
	for _, tc := range []struct {
...
// either.
func TestPiPromptFailsWhateverTheToolsRestriction(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("the image's scripts run on Linux")
	}
...
func TestPiPromptWithoutASchemaIsUntouched(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("the image's scripts run on Linux")
	}
	...
	if out != "The change looks fine.\n\nNothing else to say." {
		t.Fatalf("stdout = %q, want what pi printed", out)
	}
}

// runPrompt drives the wrapper against a pi that prints stdout and exits with
// status.
...
func stubDir(t *testing.T, pi string) string {
	t.Helper()
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "pi"), []byte(pi), 0o700); err != nil {
		t.Fatal(err)
	}
