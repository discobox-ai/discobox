...
func TestTerminalCompletionUsesSandboxScope(t *testing.T) {
	server := completionServer(t, map[string]string{
		"/api/projects/project-1/sandboxes/sandbox-1/agent-terminals": `{"terminals":[{"id":"terminal-1","agentId":"codex","status":"running","command":["codex"],"workdir":"/workspace","createdAt":"2026-01-01T00:00:00Z"}]}`,
	})
	root := NewRootCommand()
	setFlag(t, root, "server", server.URL)
	setFlag(t, root, "project", "project-1")
