...
func TestProjectFlagCompletionListsDefaultAndProjects(t *testing.T) {
	...
	assertCompletionValues(t, completions, "default", "project-1")
}

func TestSandboxPositionalCompletionListsSandboxes(t *testing.T) {
	server := completionServer(t, map[string]string{
		"/projects/project-1/sandboxes": `{"sandboxes":[{"id":"sandbox-1","projectId":"project-1","createdByUserId":"user-1","displayName":"Alpha","config":{"name":"Alpha","image":""},"runtime":{"state":"ready","runtimeState":"running","displayState":"running","desiredState":"present","generation":1,"observedGeneration":1},"createdAt":"2026-01-01T00:00:00Z","updatedAt":"2026-01-01T00:00:00Z"}]}`,
	})
	root := NewRootCommand()
...
func TestTerminalCompletionUsesSandboxScope(t *testing.T) {
	...
	setFlag(t, root, "server", server.URL)
	setFlag(t, root, "project", "project-1")
	parent := findCommand(t, root, "admin", "terminal")
	setFlag(t, parent, "discobox-id", "sandbox-1")
	cmd := findCommand(t, root, "admin", "terminal", "logs")
...
func TestExecCompletionUsesSandboxScope(t *testing.T) {
	...
	setFlag(t, root, "server", server.URL)
	setFlag(t, root, "project", "project-1")
	parent := findCommand(t, root, "admin", "exec")
	setFlag(t, parent, "discobox-id", "sandbox-1")
	cmd := findCommand(t, root, "admin", "exec", "logs")
