...
func (a *App) writeAgentDefinition(cmd *cobra.Command, definition *apimodel.AgentConfigDefinition) error {
	...
	fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\n", definition.ID, definition.Name, strings.Join(definition.RunCommand, " "), formatAgentSecrets(definition.Secrets.Or(nil)), definition.Description.Or(""))
	return tw.Flush()
}
...
func (a *App) writeAgent(cmd *cobra.Command, agent *apimodel.AgentConfig) error {
	...
	fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%s\n", agent.ID, agent.Slug, agent.Name, strings.Join(agent.RunCommand, " "), formatAgentSecrets(agent.Secrets.Or(nil)), formatTime(agent.UpdatedAt))
	return tw.Flush()
}
...
func (a *App) writeAgents(cmd *cobra.Command, agents []apimodel.AgentConfig, defaultAgentConfigID ...string) error {
	...
	for _, agent := range agents {
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%s\n", agent.ID, agent.Slug, agent.Name, formatDefaultMarker(agent.ID == defaultID), strings.Join(agent.RunCommand, " "), formatTime(agent.UpdatedAt))
	}
	return tw.Flush()
}
