...
// registered ones beside it (ADR 0116 §3). Listings span all of them, and an
...
// from before every server had one (ADR 0117).
...
// one: the name it offers (ADR 0116 §2), made into one a name may be. A server
...
func (a *App) loadServers() ([]*server, error) {
	...
	// peer ID are two spellings of one server (ADR 0116 §3), and reading it off
...
//
// It is never started — only the primary may be (ADR 0116 §3) — it works in
...
// offeredName is the name a server offers for itself (ADR 0116 §2), or empty
...
// a registered one (ADR 0116 §4). The primary is waited on as it always was; a
...
// difference matters once more than one server is asked (ADR 0116 §4): the
...
// it (ADR 0116 §6) — so a mistyped address leaves nothing behind.
