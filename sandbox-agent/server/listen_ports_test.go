...
// stages — the egress bridge's also names the DNS stub's — and a config that
// is absent names nothing.
func TestAgentListenPortsCoverEveryAgentListener(t *testing.T) {
	...
	if err := os.WriteFile(egress, []byte(`{"listenAddress":"127.0.0.1:17008","workerProxyUrl":"https://pool:17080","dnsListenAddress":"169.254.53.53:53"}`), 0o600); err != nil {
		t.Fatal(err)
	}
