...
- Homebrew ([ADR 0107](../docs/adr/0107-homebrew-is-image-content-on-an-overlay-handed-to-a-group.md)) is baked at
  ...
  and the build's ownership stands unchanged. Bottle *downloads* are shared through
  `HOMEBREW_CACHE` under `~/.cache`, so with the pool's same-uid sandboxes
  rather than pool-wide (ADR 0094); only the Cellar is per-sandbox. `HOMEBREW_NO_AUTO_UPDATE` is set for the same overlay reason:
  ...
  The hazard is narrowed rather than closed — `~/.local/bin`, `~/.cargo/bin`
  and the nix profile all still precede `/usr/local/bin`, so a `docker` CLI
