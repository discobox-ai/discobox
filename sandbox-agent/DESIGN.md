...
| `boot` | The PID-1 `init` flow: resolves the sandbox user and adds it to the manifest's `additionalGroups`, wires image-declared data/cache volumes and manifest sources from the primary volumes, binds the config volume onto `/etc/discobox`, creates the working root and gives it to the sandbox user, along with any directory it has to invent on the way to a source target (the image ships the working root root-owned, before that user exists, and a sandbox with no source has no source target under it for anything else to chown), seeds the user's `~/.gitconfig`, `~/.config/direnv/direnv.toml`, and `~/Desktop` launchers, writes desktop drop-ins, then execs the container's real init (systemd). See ADR 0007. Also the **export mode**, `discobox-sandbox-agent export <subtree>...` (`export.go`, [ADR 0129](../docs/adr/0129-the-sandbox-agent-reads-the-tree-an-export-carries.md)): PID 1 of a one-shot container the pool agent starts from the sandbox's own image to read a stopped sandbox's tree. It resolves the user and declared volumes exactly as `init` does — only the sandbox can say where a `%HOME%` path lives — then writes `data` and/or `sources` from the primary volumes to stdout as a `sandboxtree` archive, leaving out each `excludeFromExport` data path's `volumeDir`. It wires, chowns and starts nothing; the trees are mounted read-only, and only the archive goes to stdout, so everything it says goes to stderr, which the pool agent reports when it fails. |
...
| `shimproxy` | The agent's client of a shim's Unix socket: status reads on a short dial timeout, JSON calls (`CallJSON`), and bridging an exec attach through to the caller as an HTTP upgrade or a websocket (`AttachHTTPUpgrade`, `AttachWebSocket`). |
| `hooks` | Local Unix-socket collector and publisher protocol for coding-harness lifecycle payloads. Arbitrary run users publish through a dedicated sibling runtime directory (0711) and writable socket (0666), while terminal attach sockets remain private. These events are not an authorization boundary: anything in the sandbox may publish one. They are recorded for `discobox admin audit hooks` to read back and for a terminal wait to match (ADR 0137 §3). The collector records the hook under the name its harness used; the canonical name beside it is the store's to derive, on the way in and in one place (ADR 0146 §4, `harness.CanonicalHookEvent`). |
...
| `dnsstub` | The sandbox's DNS stub, run inside the sandbox's proxy bridge (`proxy-bridge --dns`, set in `discobox-proxy-bridge.service` and not in the nested-Docker instance): it reaches the pool with the same keypair as egress, and its failing is logged rather than stopping the bridge. The container is created with a link-local DNS server the pool names in `bridge.json` (`dnsListenAddress`); the stub claims that address on `lo`, drops anything for it arriving on another interface (a neighbor could route it through this sandbox and be asking under this sandbox's certificate), and listens there on UDP and TCP 53, so Docker's embedded resolver hands it every name it cannot answer without the query leaving the sandbox. It asks the pool's `dnsServer` as DNS over TLS with the sandbox's client certificate, verifying the pool's, over at most `maxConns` connections kept open between queries — under the pool's per-sandbox admission, so a burst waits for a connection instead of being refused; trims an answer too large for a UDP client to a truncated reply; and answers SERVFAIL, without dialing, for the pool's own name, which reaches it only when the pool is off the network. A `bridge.json` from a pool that serves no DNS names neither field and the stub exits cleanly ([ADR 0148](../docs/adr/0148-a-sandbox-resolves-names-through-its-pool-over-mtls.md)). |
| `proxyenv` | Renders `sandbox.json`'s proxy-trust env (`Env`/`ProxyEnvs`) as a systemd `EnvironmentFile`, for `docker.service`, `nix-daemon.service` and the Xfce session (`xfce4-session@.service`) — all started by systemd, not spawned by sandbox-agent, so they inherit no container env and cannot be reached by any per-container injection. The two daemons do their own fetching (image pulls; substitutions and builds the `nix` client hands off), so their egress is the sandbox's egress; the session passes the env to everything launched from the desktop, a browser included. `NO_PROXY` is resolved once, at boot, so a network created later (the default Docker bridge, which appears when dockerd first starts) is missing from it. Harmless for the daemons, which only fetch off-box; for the desktop it means a container's bridge IP is sent to the proxy. Resolves `sandboxconfig.LocalSubnetsToken` against `nestedbridge.LocalSubnets()`, the same substitution `runcca.proxyEnv` applies for nested containers. Run at boot by `discobox-render-proxy-env.service`, ordered before both daemons, writing to `/run/discobox/proxy/proxy.env` (not `/etc/discobox`, which is pool-agent's read-only mount). |
...
| `store` | Sandbox-local SQLite/GORM audit log, exec records and observed state snapshots, retained resource blobs, harness hook events (what `discobox admin audit hooks` reads), each under the name its harness used and the canonical name for it when Claude Code has one (ADR 0146), filled for rows written before the column by a backfill on open, and exec events, both readable across terminals and execs with a `since` bound and forward order for a follower and an `until` bound for a reader paging back, the primary-terminal launched marker (`AgentState`), and compressed exec/terminal transcript chunks (see ADR 0028). |
| `image.json` | The base manifest layer every harness image inherits (ADR 0086 §2), baked in as the `io.discobox.image.v1.10-sandbox-base` label: the env and PATH (including `DISPLAY` and the session bus's `DBUS_SESSION_BUS_ADDRESS`, which go together — see [`desktop`](desktop/DESIGN.md#one-session-bus)), the data/cache volumes `boot` wires, the `additionalGroups` the sandbox user joins (`brew`, `docker`, `kvm`, and `systemd-journal`, so a Discobox server run inside a sandbox can read its Docker daemon's journal for `pool logs`), and seed harness files. A group grant on a host device such as `/dev/kvm` lands only because the image's GID for that group matches the host's — both are the same Debian release installing the same packages. |
...
| `image/discobox-shell` | Installed as `/usr/local/bin/discobox-shell`: the launcher `execs.PreferredShellCommand` runs for a shell a client asked for by name (ADR 0138). Takes `PREFERRED FALLBACK ARG...`; looks for the preferred path, else its basename on PATH, in a subshell that has sourced `/etc/profile` (so `sandbox-direnv.sh` has exported the starting directory's `.envrc`), and `exec`s what it found with `SHELL` set to it, else the fallback, after setting the terminal title to that shell's name. zsh, nu and any other shell that reads no `/etc/profile` start with the loaded environment; the fallback and any shell that reads `/etc/profile` itself (bash, sh, dash, ksh, mksh) start from the one the launcher was given, because a second profile run resets PATH and direnv, seeing the `.envrc` already loaded, would not restore it. |
| `image/shells` | What makes zsh and nu (ADR 0138) as usable on first start as bash. `zsh-newuser` is installed as `/usr/share/zsh/scripts/newuser`, the site-wide script zsh's newuser module prefers to its own: for a user with no zsh startup files it seeds Debian's `newuser.zshrc.recommended` instead of starting the `zsh-newuser-install` questionnaire. `nushell-direnv.nu` is nu's direnv pre-prompt hook, in `/usr/local/share/nushell/vendor/autoload`. zsh is a Debian package; nu is the pinned upstream static musl build, per-architecture checksums in the Dockerfile. |
| `image/services` | The image's own service declarations, installed at `/usr/local/share/discobox/services` for `services` to read: `10-desktop.yaml` declares `ai.discobox.desktop` on 6900 with `protocol: http` and nothing to run (a `.yaml` service starts never), because `discobox-desktop.socket` already serves it. |
| `image/tools` | The image's own tool declarations, installed at `/usr/local/share/discobox/tools`: `10-diff.yaml` runs `discobox-review` and `20-fresh.sh` records fresh's workspace trust and runs `fresh`, whose carried files' defaults sit beside it under `fresh/`. The agent only lists them — with the primary source's `.discobox/tools` — at `GET .../tools` (`server/tools.go`), re-read per request; which tools exist and run is the client's merge ([ADR 0125](../docs/adr/0125-tools-are-declared-in-files-the-way-services-are.md)). |
...
- The desktop's density and cursor size are written to xfconf's `xsettings`
  ...
  refresh. That is also why the desktop shares `discobox-session-bus.service`:
...
- Any code that reads `sandbox.json`'s `Env` to actually launch or configure
  ...
  and `proxyenv.Render` are the three current call sites. `proxyenv.Render` is
  the known exception: it runs once at boot, so the long-lived units that read
  its file (dockerd, nix-daemon, the Xfce session) see the networks that existed
  then, and not the default Docker bridge, which appears later. Only the desktop
  session is affected, because it launches browsers that may address a
  container by its bridge IP.
...
- Treat systemd as the source of truth for terminal unit liveness. Runtime JSON
  ...
  writing its own exit — and when systemd reports a change to its unit, which is
  the only way to see an end the shim did not survive. It sweeps once at
  startup, and on an interval only when no subscription could be established.
  The shim's write is the sole accurate source of an **exit status**: a
  transient unit is collected as it dies, so its `ExecMainStatus` reads zero by
  the time anything can query it. What systemd is authoritative for is whether
  the unit is still *there*. Durable database-only exec records must be reconciled
...
- `ports` is that exception, and the reason is specific to it
  ...
  reports its snapshot. Discovery is a `/proc/net/{tcp,udp}{,6}` read filtered
...
- `execs.Manager.Stop` is not `Delete`. Stop ends the run, removes the shim's
  ...
  tmpfs, so after a reboot the record is all there is. Without it a sweep finds
  the unit gone and calls the exec `lost`, which is true of a unit that vanished
...
shared base, pool, base sandbox, Codex, Claude Code, OpenCode, Pi, Oh My Pi, and Shell images. Each harness
...
- **What a sandbox exists for decides what starts in it** (`config.WorkedIn`).
  ...
  `judge`, the project's judge ([ADR 26-09-22-838](../docs/adr/26-09-22-838-a-dedicated-pool-harness-judges-commands-and-credential-bearing-requests.md)).
...
- A source tree's `.envrc` loads without anyone typing `direnv allow`, in two
  ...
  interactive shells `/etc/profile` does not reach. zsh and nu, which read
  none of that, get their own: `direnv hook zsh` appended to `/etc/zsh/zshrc`,
  and `image/shells/nushell-direnv.nu` in nu's vendor autoload directory
  (ADR 0138). The trust is per-sandbox, so
...
- A client may still say which shell its *person* prefers: `DISCOBOX_SHELL`
  ...
  shell — by running the image's `discobox-shell <preferred> <login shell> -l`,
  unless the preference names the login shell itself (same path or name),
  which runs unwrapped so the record names it.
  The login shell is still `ResolveShell`'s answer, passed as the fallback; the
  launcher finds the preferred one after the login profile and `.envrc` load.
  Never read from the merged env, so an image or manifest cannot set it.
...
- A TTY exec is also read, typed into, and waited on without attaching
  ([ADR 0137](../docs/adr/0137-a-terminal-can-be-read-typed-into-and-waited-on-without-attaching.md)):
  `.../screen`, `.../input` and `.../wait` (`server/terminal_io.go`) each ask
  the shim, which alone holds the emulator, the PTY, and output as it happens.
  The screen is text rendered from the emulator's cells, never the raw stream.
  Input is encoded against the modes the program set (bracketed paste,
  application cursor keys) and counts as access, like an attach's input frames.
  The shim answers quiet and exit. Quiet counts from the latest of output,
  input, and the wait's start, so a wait right after typing cannot be satisfied
  by output that predates it. Exit is the shim's own `exited` signal, since
  `done` closes only after the post-exit linger, and a screen read during that
  linger says `exited`. Meanwhile the agent watches the
  hook record for the named hook events (`store.HarnessHookSignal`,
  `FirstHarnessHookSince`, which matches the events in the query so a turn's
  end is found behind any number of tool-call hooks, and matches a name against
  either a hook's own event or its canonical one, so `Stop` ends a wait on
  every harness that has a turn-end hook — ADR 0146 §6), and the first to hold
  ...
  the sandbox runs (`requireRunning`); reading never starts it. A wait is also the one route that outlasts the shim server's own
  write timeout, so it sets a deadline of its own: without it a wait that holds
  longer answers nothing and the caller reads the closed connection as an
  unexpected EOF.
...
- The screen also records when what the program shows last changed — the
  screen's text, or the title to a *different* value — reported on the shim's
  `/status` as `screenChangedAt` and cleared with the run like
  `lastAccessedAt` ([ADR 0124](../docs/adr/0124-a-terminal-is-active-while-what-it-shows-changes.md)).
  The text is compared as a hash of cell content and the alternate-screen
  flag — never styles, the cursor, or the scrollback, which the emulator fills
  on every full-screen erase — so a redraw of the same screen, a self-drawn
  blinking cursor, or a title re-sent on redraw is not a change, and a resize
  re-baselines without counting. The screen is compared once output pauses
  for 50ms (by the next write or `/status` read), not part-way through a frame
  split across PTY reads, and at least every second of output that never
  pauses; a change is dated to the burst's last write. A still screen is what
  the idle stop reads as a program that is not busy.
