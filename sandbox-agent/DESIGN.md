...
| `nrica` | The sandbox's NRI plugin (`cmd/discobox-nri-ca`): on every container a nested Docker daemon creates, mounts the sandbox's CA trust bundles and injects proxy-trust env, so a user's Dockerfile or `docker run` never needs MITM awareness. See ADR 0015. Bundles are staged per boot by `discobox-trust-ca.service` under `/run/discobox/proxy/ca-bundles`; they cannot live beside the CA in `/etc/discobox/proxy`, which is pool-agent's read-only mount. |
