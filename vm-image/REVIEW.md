...
- **Nothing here may name a backend.** No build arg, no `TARGETARCH` branch, and
  no unit that only one hypervisor can satisfy. A difference a backend genuinely
  needs and the other cannot take belongs in a separate artifact with its own
  release line, as the libkrunfw-patched kernel does (ADR 0101 §3). The one
...
- **Disk order is a contract.** Root, data and cache are `vda`, `vdb`, `vdc`,
  ...
  names. Both drivers hard-code the order against it — `vzvm.Start`, and
  `krun_add_disk3`'s fixed sequence — so reordering, inserting a disk, or
  changing the script to mount by label silently mounts the wrong filesystem on
  both backends at once. It is one script now; it used to be two.
- **A new guest device is three edits, not one.** The image ships drivers for
  the virtio devices a driver attaches and deletes the rest, so attaching a
  device the guest has never had also means keeping its module class in
  `Dockerfile` and, if the root depends on it, adding it to
...
- **Guest image changes are a separate release.** Editing this directory does
  ...
  `guestimage.DefaultVMImage` is re-pinned to the new `discobox-vm` digest. The
  kernel is its own line again, `vm-kernel/v*`.
